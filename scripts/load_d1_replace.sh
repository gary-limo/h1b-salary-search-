...
#   2. Drop remote h1b_wages table (destructive)
...
echo "Step 1/5: Exporting h1b_wages INSERT statements from local D1..."
...
echo ""
echo "Step 2/5: Dropping remote app tables..."
npx wrangler d1 execute "$DB_NAME" --remote --command "
DROP TABLE IF EXISTS h1b_wages;
"

echo ""
echo "Step 3/5: Recreating h1b_wages schema..."
npx wrangler d1 execute "$DB_NAME" --remote --file=./migrations/0001_create_h1b_wages.sql

echo ""
echo "Step 4/5: Loading h1b_wages data (this may take several minutes)..."
npx wrangler d1 execute "$DB_NAME" --remote --file=./h1b_wages_export.sql

echo ""
echo "Step 5/5: Recreating composite indexes..."
