...
async function fetchPage(emp, job, loc) {
  ...
  const res = await dedupeFetch(`/api/search?${params}`, {
    credentials: "include",
...
async function fetchApiSuggest(field, q, contextEmployer, contextJob) {
  ...
  const res = await dedupeFetch(`/api/suggest?${params}`, {
    credentials: "include",
    headers: { Accept: "application/json" },
  });
...
async function ensureApiSuggestReady(force = false) {
  ...
  suggestProbeInFlight = (async () => {
    try {
      const res = await dedupeFetch("/api/suggest?field=employer&q=", {
        credentials: "include",
        headers: { Accept: "application/json" },
      });
      if (res.ok) {
...
function acGetContext(field) {
  const ctx = {};
  if (field !== "employer" && selEmp) ctx.employer = selEmp;
  if (field !== "job"      && selJob) ctx.job      = selJob;
...
function acSetup(input, dropdownId, field) {
  ...
  async function fetchSuggest(q) {
    const hasCtx = (field === "employer" && selJob) || (field === "job" && selEmp);
    ...
    acTimers[field] = setTimeout(async () => {
      try {
        const results = await fetchApiSuggest(field, q, selEmp || undefined, selJob || undefined);
